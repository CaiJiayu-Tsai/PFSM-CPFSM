...
/**
 * This is an implementation of the PFSM algorithm <br/>
 * <br/>
 *
 * The PFSM algorithm is described in the paper: <br/>
 * <br/>
 *
 ...
 * <br/>
 * 
 * The PFSM algorithm finds all the periodic frequents subgraphs in a graph database provided by the user. <br/>
 * <br/>
 * 
 * This implementation saves the result to a file
 */
public class AlgoPFSM {

	/** the minimum support represented as a count (number of subgraph occurrences)*/
	private int minSup;

	/** the database size (number of graphs) */
	protected int databaseSize = 0;

	/** the maxper of the graph */
	int maxperiod = 0;

	/** the minper of the graph */
	int minperiod = 0;

	/** the acgper of the graph */
	double averageperiod = 0;

	/** the support pruning Threshold */
	protected double supportPruningThreshold = 0;

	/** The list of periodic frequent subgraphs found by the last execution */
	private List<FrequentSubgraph> frequentSubgraphs;

	/** runtime of the most recent execution */
	long runtime = 0;

	/** memory of the most recent execution */
	double maxmemory = 0;
	...
	/** minimum average periodicity threshold **/
	int minAveragePeriodicity;

	/** maximum average periodicity threshold **/
	double maxAveragePeriodicity;

	/**
	 * Run the PGSpan algorithm
	 * 
	 ...
	 * @param minAveragePeriodicity the minimum average periodicity threshold
	 * @param maxAveragePeriodicity the maximum average periodicity threshold
	 ...
	 */
	public void runAlgorithm(String inPath, String outPath,  int minPeriodicity,
							 int maxPeriodicity, int minAveragePeriodicity,
							 double maxAveragePeriodicity, boolean outputSingleVertices,
		...
		// Calculate the supportPruningThreshold
		databaseSize = graphDB.size();
		supportPruningThreshold  = (((double)databaseSize+1d) / ((double)maxAveragePeriodicity) ) - 1d ;
		minSup = (int)Math.round(supportPruningThreshold);
	...
	private void writeResultToFile(String outputPath) throws IOException {
		...
		for (FrequentSubgraph subgraph : frequentSubgraphs) {
			...
			sb.append(subgraph.averagePeriodicity).append(" ").append(subgraph.smallestPeriodicity)
					.append(" ").append(subgraph.largestPeriodicity).append(System.lineSeparator());
			if (dfsCode.size() == 1) {
			...
			// If the user choose to output the graph ids where the frequent subgraph appears
			// We output it
			if (outputGraphIds) {
				sb.append("x");
	...
	/**
	 * Find all isomorphisms between graph described by c and graph g
	 * each isomorphism is represented by a map
	 * 
			...
			for (Map<Integer, Integer> iso : isoms) {
				...
				if (v1 < v2) {
					...
					for (Vertex mappedV2 : g.getAllNeighbors(mappedV1)) {
						...
						// If the neighbor has the same label as V2 and is not already mapped
						// and the edge label is the same as that between v1 and v2.
						if (v2Label == mappedV2.getLabel() && (!mappedVertices.contains(mappedV2.getId()))
							...
							// because there may exist multiple extensions,
							// need to copy original partial isomorphism
			...
			for (Map<Integer, Integer> isom : isoms) {
				...
				for (Vertex x : g.getAllNeighbors(mappedRM)) {
					...
					if (invertedX != null && c.onRightMostPath(invertedX) && c.notPreOfRM(invertedX)
						...
						// rightmost and invertedX both have correspondings in g,
						// so label of vertices and edge all can be found by correspondings
				...
				for (Map<Integer, Integer> isom : isoms) {
					...
					for (Vertex x : g.getAllNeighbors(mappedRM)) {
						...
						if (invertedX != null && c.onRightMostPath(invertedX) && c.notPreOfRM(invertedX)
							...
							// and edge all can be found by correspondings
							ExtendedEdge ee = new ExtendedEdge(rightMost, invertedX, mappedRMlabel, x.getLabel(),
									g.getEdgeLabel(mappedRM, x.getId()));
							if (extensions.get(ee) == null)
								extensions.put(ee, new HashSet<>());
			...
			for (int i = 0; i < vertices.length; i++) {
				...
				for (Edge edge : v1.getEdgeList()) {
					...
					if (ELIMINATE_INFREQUENT_EDGE_LABELS) {
						...
						if (!seen) {
							matrix.incrementCount(labelV1, labelV2, g.getId());
			...
			for (Graph g : graphDB) {
				...
				for (int i = 0; i < vertices.length; i++) {
					...
					while (iter.hasNext()) {
						...
						int maxcount = matrix.getMaxperForItems(labelV1, labelV2,graphDB.size());

						if (ELIMINATE_INFREQUENT_VERTEX_PAIRS && (count < minSup || maxcount > maxPeriodicity)) {
	...
	private void gSpanDFS(DFSCode c, List<Graph> graphDB, Set<Integer> graphIds)
		...
		// the list of graph ids where this edge extends the current subgraph c.
		...
		if (extensions != null) {
			for (Entry<ExtendedEdge, Set<Integer>> entry : extensions.entrySet()) {
				...
				// Get the maxper minper and avgper of the newgraph
				List<Integer> listperiodset = getlistperiodset(newGraphIDs, graphDB);
				maxperiod = Collections.max(listperiodset);
				minperiod = Collections.min(listperiodset);
				averageperiod = ((double) databaseSize+1) / ((double) newGraphIDs.size()+1) ;

				// If the newgraph satisfy the period and frequency
				if(averageperiod <= maxAveragePeriodicity
						&& averageperiod >= minAveragePeriodicity
						&& minperiod >= minPeriodicity
						&& maxperiod  <= maxPeriodicity
						&& sup >= minSup){
					...
					if (isCanonical(newC)) {
						...
						subgraph.largestPeriodicity = maxperiod;
						subgraph.smallestPeriodicity = minperiod;
						subgraph.averagePeriodicity = averageperiod;

						// Add into the periodic frequent set
				...
				// Pruning  condition
				else if(sup >= supportPruningThreshold
						&& maxperiod <= maxPeriodicity
						&& sup >= minSup){
	...
	private void findAllOnlyOneVertex(List<Graph> graphDB, boolean outputFrequentVertices) {
		...
		for (Entry<Integer, Set<Integer>> entry : labelM.entrySet()) {
			...
			// calculate the parameter of the vertex
			List<Integer> tempSupGperiodset = getlistperiodset(tempSupG, graphDB);
			maxperiod = Collections.max(tempSupGperiodset);
			minperiod = Collections.min(tempSupGperiodset);
			averageperiod = ((double) databaseSize+1) / ((double) tempSupG.size()+1) ;
			int sup = tempSupG.size();

			// estimate the vertex is frequent and periodic

			if (sup >= supportPruningThreshold
					&& maxperiod <= maxPeriodicity) {
				...
				if (outputFrequentVertices) {
					...
					// create a new subgraph to store the vertex
					FrequentSubgraph subgraph = new FrequentSubgraph(tempD, tempSupG, sup);
					subgraph.largestPeriodicity = maxperiod;
					subgraph.smallestPeriodicity = minperiod;
					subgraph.averagePeriodicity = averageperiod;
				...
				// the vertex is not frequent or periodic, then remove
			} else if (ELIMINATE_INFREQUENT_VERTICES
							&& (sup < supportPruningThreshold
							|| maxperiod > maxPeriodicity)) {
				// for each graph
	...
	/**
	 * Caculate the maxper minper and avgper
	 * @param newGraphIDs
	 * @param graphDB
	 * @return
	 */
	 public List<Integer> getlistperiodset(Set<Integer> newGraphIDs, List<Graph> graphDB) {

	 List<Integer> listGraphIDs = new ArrayList<Integer>(newGraphIDs);
	 Collections.sort(listGraphIDs);
	 ...
	 listperiodset.add(graphDB.size()-listGraphIDs.get(listGraphIDs.size()-1));

	 return listperiodset;
	 }
	...
	public void printStats() {
		System.out.println("=============  PFSM - STATS =============");
		System.out.println(" Number of graph in the input database: " + graphCount);
		System.out.println(" Periodic frequent subgraph count : " + patternCount);
		System.out.println(" Total time ~ " + runtime + " ms");
		System.out.println(" Minsup : " + minSup + " graphs");
		System.out.println(" MaxPer : " + maxPeriodicity);
		System.out.println(" MaxAvg : " + maxAveragePeriodicity);
		System.out.println(" MinPer : " + minPeriodicity);
		System.out.println(" MinAvg : " + minAveragePeriodicity);
